...
//  Client
//
//  Created by Farhan Patel on 12/28/17.
...
enum DeepLink {
    ...
    init?(urlString: String) {
        if urlString.startsWith("/settings/"), let link = SettingsPage(rawValue: urlString.replacingOccurrences(of: "/settings/", with: "")) {
            self = .settings(link)
        } else if urlString.startsWith("/homepanel/"), let link = HomePanelPath(rawValue: urlString.replacingOccurrences(of: "/homepanel/", with: "")) {
            self = .homePanel(link)
        } else {
            return nil
        }
...
enum NavigationPath {
    ...
    init?(url: URL) {
        ...
        if urlString.startsWith("\(scheme)://deep-link"), let deepURL = components.valueForQuery("url"), let link = DeepLink(urlString: deepURL) {
            self = .deepLink(link)
        } else if urlString.startsWith("\(scheme)://fxa-signin"), components.valueForQuery("signin") != nil {
            self = .fxa(params: FxALaunchParams(query: url.getQuery()))
        } else if urlString.startsWith("\(scheme)://open-url") {
    ...
    static func handleFxA(params: FxALaunchParams, with bvc: BrowserViewController) {
        if AppConstants.MOZ_FXA_DEEP_LINK_FORM_FILL {
            bvc.presentSignInViewController(params)
        }
    }
    
    static func handleHomePanel(panel: HomePanelPath, with bvc: BrowserViewController) {
        switch panel {
    ...
    static func handleURL(url: URL?, isPrivate: Bool, with bvc: BrowserViewController) {
        if let newURL = url {
    ...
    static func handleSettings(settings: SettingsPage, with rootNav: UINavigationController, baseSettingsVC: AppSettingsTableViewController, and bvc: BrowserViewController) {

...
class AppDelegate: UIResponder, UIApplicationDelegate, UIViewControllerRestoration {
    ...
    func application(_ application: UIApplication, open url: URL, sourceApplication: String?, annotation: Any) -> Bool {
        ...
        handle(nav: routerpath)
        return true
    }
    
    func handle(nav: NavigationPath) {
        switch nav {
        case .fxa(let params): NavigationPath.handleFxA(params: params, with: self.browserViewController)
        case .deepLink(let link): handleDeepLink(link)
        case .url(let url, let isPrivate): NavigationPath.handleURL(url: url, isPrivate: isPrivate, with: self.browserViewController)
        }
    }
    ...
    func handleDeepLink(_ link: DeepLink) {
        ...
        case .homePanel(let panelPath):
            NavigationPath.handleHomePanel(panel: panelPath, with: self.browserViewController)
        case .settings(let settingsPath):
            guard let rootVC = rootViewController as? UINavigationController else {
                return
            }
            ...
            NavigationPath.handleSettings(settings: settingsPath, with: rootVC, baseSettingsVC: settingsTableViewController, and: browserViewController)
        }
    }
